...
get_rv_prebuilts() {
	...
	for src_ver in "$cli_src CLI $cli_ver revanced-cli" "$patches_src Patches $patches_ver patches"; do
		...
		if [ "$tag" = "Patches" ]; then
			if [ $grab_cl = true ]; then echo -e "[Pathces Changelog](https://github.com/${src}/releases/tag/${tag_name})\n" >>"${cl_dir}/changelog.md"; fi
...
get_archive_vers() { sed 's/^[^-]*-//;s/-\(all\|arm64-v8a\|arm-v7a|armeabi-v7a\)\.apk//g' <<<"$__ARCHIVE_RESP__"; }
